...
import { View, Share, StyleSheet, Platform, Dimensions, ScrollView } from 'react-native';
...
class Connec extends React.Component {
  ...
  shareVCard()  {
    ...
    var uri2 = FileSystem.documentDirectory + this.fname + '_' + this.lname + '.vcf'
    if(Platform.OS == 'ios'){
      const result = Share.share({
        url: uri
      })
    } else {
      const result = Sharing.shareAsync(
          uri2,
          {mimeType: 'text/vcard'}
          );
//       IntentLauncherAndroid.startActivityAsync('android.intent.action.PICK_CONTACT', {URI : uri});
  ...
  render() {
    ...
    this.renderShare =  true //(Platform.OS === 'ios')
