...
class Hotels extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({ hotels: hotelData.data.listHotels.items, loading: false })
    } catch (err) {
      console.log('err: ', err)
          ...
          <View style={styles.listContainer}>
          {
            hotelData.map((hotel, i) => (
              <TouchableOpacity
                key={i} 
                onPress={
                  () => this.props.navigation.push('Hotelview', hotel)
                }
              >
                  ...
                  <View style={styles.speakerContainer}>
                    ...
                    <View style={styles.infoContainer}>
                      <Text
                        style={styles.name}
                      >{hotel.name}</Text>
                      <Text style={styles.speakerName}>{hotel.shortDesc}</Text>
                    </View>
                  </View>
                  <View style={styles.timeContainer}>
                    <Text style={styles.hotelTime}>{hotel.website}</Text>
                  </View>
                </View>
              </TouchableOpacity>
            ))
